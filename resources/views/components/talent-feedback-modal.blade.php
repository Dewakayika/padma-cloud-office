@props(['project', 'feedbackExists' => false, 'companySlug' => null])
...
<div x-data="{ open: true }" x-show="open" class="fixed inset-0 z-[9999] flex justify-center items-center" style="display: none;">
    ...
    <div class="relative w-full max-w-lg bg-white rounded-xl shadow-2xl z-[10000] p-8">
        ...
        <h2 class="text-xl font-bold mb-4">Give Feedback & Rate Project</h2>
        <form method="POST" action="{{ route('talent.project.feedback', [$companySlug, $project]) }}">
            @csrf
            <div class="mb-4">
                <label class="block text-gray-700 font-semibold mb-2">Feedback</label>
                <textarea name="feedback_text" rows="4" required class="w-full border rounded px-3 py-2 focus:outline-none focus:ring focus:border-blue-300" placeholder="Share your experience with this project..."></textarea>
            </div>
            <div class="mb-4" x-data="{ rating: 0 }">
                ...
                <p class="text-sm text-gray-500 mt-1">Click on the stars to rate this project</p>
            </div>
