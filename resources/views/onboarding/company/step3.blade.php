...
<div class="max-w-4xl mt-10 sm:mt-14 mx-auto mb-8 bg-gray-50 rounded-xl shadow-sm px-4 md-px-0 py-6">
...
<div class="max-w-4xl mx-auto bg-white shadow rounded-xl p-8 mb-8">
    ...
    <form method="POST" action="{{ route('company.onboarding.step.post', ['step' => 3]) }}">
        ...
        <div class="mb-5">
            ...
            <select name="primary_use_case" class="form-input w-full bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-orange-200" required>
                <option value="" class="text-sm md:text-md" disabled selected>Select your primary use case</option>
                <option value="Full Production Support" class="text-sm md:text-md" @if(old('primary_use_case', $company->primary_use_case) == 'Full Production Support') selected @endif>Full Production Support</option>
                <option value="Character Design" class="text-sm md:text-md" @if(old('primary_use_case', $company->primary_use_case) == 'Character Design') selected @endif>Character Design</option>
                <option value="Background Art Creation" class="text-sm md:text-md" @if(old('primary_use_case', $company->primary_use_case) == 'Background Art Creation') selected @endif>Background Art Creation</option>
                <option value="Line Art Assistance (Clean-Up / Inking)" class="text-sm md:text-md" @if(old('primary_use_case', $company->primary_use_case) == 'Line Art Assistance (Clean-Up / Inking)') selected @endif>Line Art Assistance (Clean-Up / Inking)</option>
                <option value="Coloring (Flat, Shadow, or Full Render)" class="text-sm md:text-md" @if(old('primary_use_case', $company->primary_use_case) == 'Coloring (Flat, Shadow, or Full Render)') selected @endif>Coloring (Flat, Shadow, or Full Render)</option>
                <option value="3D Asset Design" class="text-sm md:text-md" @if(old('primary_use_case', $company->primary_use_case) == '3D Asset Design') selected @endif>3D Asset Design</option>
                <option value="Layout Assistance" class="text-sm md:text-md" @if(old('primary_use_case', $company->primary_use_case) == 'Layout Assistance') selected @endif>Layout Assistance</option>
                <option value="Other" class="text-sm md:text-md" @if(old('primary_use_case', $company->primary_use_case) == 'Other') selected @endif>Other</option>
            </select>
        </div>
        <div class="mb-10">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-4">
                ...
                @foreach($tools as $tool)
                    <label class="flex items-center gap-3 bg-gray-50 border border-gray-100 rounded-lg px-4 py-3 cursor-pointer transition shadow-none hover:shadow-md hover:bg-white focus-within:ring-2 focus-within:ring-blue-200">
                        <input type="checkbox" name="collaboration_tools[]" value="{{ $tool }}" @if(in_array($tool, $selected)) checked @endif class="accent-orange-500 w-5 h-5 rounded border-gray-300 focus:ring-2 focus:ring-blue-200">
        ...
        <div class="bg-blue-50 border-l-4 border-blue-400 p-4 rounded mb-6">
            ...
            <p class="text-sm text-blue-700 mb-2">To ensure confidentiality when working with Indonesian talent, please review and sign our standard NDA. This agreement will apply to your studio and all talent who join your projects.</p>
            <button type="button" class="text-blue-600 underline text-xs font-medium mb-2">Review NDA Document</button>
            <label class=" items-center mt-2 flex">
        ...
        <div class="bg-orange-50 border-l-4 border-orange-400 p-4 rounded mb-6">
            ...
            <ul class="text-sm text-orange-700 list-disc pl-5">
